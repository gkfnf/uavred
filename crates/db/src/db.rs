...
/// Get the default database directory for UAVRed
pub fn database_dir() -> std::path::PathBuf {
    dirs::data_dir()
        .unwrap_or_else(|| std::env::temp_dir())
        .join("uavred")
}

/// Check if running in stateless mode (no persistent database)
pub fn is_stateless() -> bool {
    std::env::var("UAVRED_STATELESS").is_ok()
}
...
pub async fn open_db<M: Migrator + 'static>(db_dir: &Path, scope: &str) -> ThreadSafeConnection {
    ...
    let connection = try_open_db::<M>(&main_db_dir).await;
...
async fn try_open_db<M: Migrator>(db_dir: &Path) -> Option<ThreadSafeConnection> {
    smol::fs::create_dir_all(db_dir)
        .await
        .context("Could not create db directory")
        .ok()?;
    let db_path = db_dir.join(Path::new(DB_FILE_NAME));
...
/// Write to database and log errors
pub fn write_and_log<F>(db_write: impl FnOnce() -> F + Send + 'static)
where
    F: Future<Output = anyhow::Result<()>> + Send,
{
    smol::spawn(async move {
        if let Err(e) = db_write().await {
...
mod tests {
    ...
    fn test_bad_migration_panics() {
        ...
        let tempdir = tempfile::Builder::new()
            ...
            .tempdir()
            .unwrap();
        let _bad_db = smol::block_on(open_db::<BadDB>(
            tempdir.path(),
            "dev",
        ));
    }
    ...
    fn test_db_corruption() {
        ...
        let tempdir = tempfile::Builder::new()
            ...
            .unwrap();
        {
            let corrupt_db = smol::block_on(open_db::<CorruptedDB>(
                tempdir.path(),
                "dev",
            ));
    ...
    /// Test that DB exists but corrupted (causing recreate)
    #[test]
    fn test_simultaneous_db_corruption() {
        enum CorruptedDB {}
