...
impl Connection {
    ...
    pub fn sql_has_syntax_error(&self, sql: &str) -> Option<(String, usize)> {
        ...
        while {
            ...
            // Only report syntax errors (not runtime errors like "no such table")
            // Syntax errors have code 1 (SQLITE_ERROR) and don't contain specific phrases
            if res == 1 && offset >= 0 {
                ...
                // Filter out runtime errors that are not syntax errors
                const NON_SYNTAX_ERRORS: &[&str] = &[
                    "no such table",
                    "no such column",
                    "no such index",
                    "no such view",
                ...
                let is_syntax_error = !NON_SYNTAX_ERRORS.iter()
                    .any(|&prefix| err_msg.to_lowercase().contains(prefix));
